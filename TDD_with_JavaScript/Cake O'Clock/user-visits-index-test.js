...
describe('User visits index', () => {
  describe('to post an order', () => {
    ...
    it('Pick up time is displayed in #:00 format in the order ticket after submission', () => {
      const timeExpected = '9:00';
      browser.url('/');
      browser.selectByVisibleText('#select-pickUp', timeExpected);
      browser.click('#submit-order');
      browser.url('/');
      
      assert.include(browser.getText('#pickUp'), timeExpected);
    });
    
    // Add the 'labels the pick up hour' test here
		it('The field in the order ticket is labeled “Pick up time:”', () => {
      browser.url('/');
      const HTML = browser.getHTML('body');
      const parsedHTML = parseTextFromHTML(HTML, '#pickUp');
      
      assert.include(parsedHTML, 'pick up time:');
    });
